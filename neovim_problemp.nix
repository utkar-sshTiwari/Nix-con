...

{
  programs.neovim = {
    ...
    plugins = with pkgs.vimPlugins; [
      ...
      # ── Treesitter — listed as a bare plugin, NO config block here ───────────
      # setup() is called in extraLuaConfig below, which runs after ALL plugins
      # are added to the runtimepath — this is the only reliable order on NixOS
	nvim-treesitter
